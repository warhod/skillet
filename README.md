...
```bash
# Install from .skillet/config/sources.json (bootstraps defaults on first run); prompts once for IDE targets
skillet install
...
4. **Register and materialize it**:
   ...
   ```
   That appends a `local` entry to `.skillet/config/sources.json`, copies the skill under `.skillet/skills/`, and refreshes the native skill directories for your enabled targets (see [Where skills are discovered](#where-skills-are-discovered)).
